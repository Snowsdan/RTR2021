...
static const glm::vec3 DEFAULT_POSITION = glm::vec3(0.0f, 10.0f, 0.0f);
...
static const float DEFAULT_PITCH        = -90.0f;          // degrees
...
public:
    ...
    void TiltUp(unsigned int td_milli) { 
        if (m_Pitch < -90.0) {
            m_Pitch += m_TiltSpeed * td_milli / 1000.0f; UpdateVectors();
        }  
        else {
            m_Pitch = -90.0; UpdateVectors();
        }
